...
(function() {
  ...
  Migrator = (function() {
    ...
    Migrator.prototype._collectRanMigrations = function(cb) {
      this._ranMigrations = {};
      return this._coll().find().toArray((function(_this) {
    ...
    Migrator.prototype._runWhenReady = function(direction, cb, progress) {
      ...
      onSuccess = (function(_this) {
        return function() {
          return _this._collectRanMigrations(function(err) {
            if (err) {
              return cb(err);
            } else {
    ...
    Migrator.prototype.isUpToDateFromDir = function(dir, done) {
      return this._loadMigrationFiles(dir, (function(_this) {
        return function(err, migrationFiles) {
          var makeDiffWhenDbReady;
          if (err) {
            return done(err);
          } else {
            makeDiffWhenDbReady = function() {
              return _this._collectRanMigrations(function(err) {
                var differenceCount, migrationFilesIds, ranMigrationsIds;
                if (err) {
                  return done(err);
                } else {
                  migrationFilesIds = _.map(migrationFiles, 'module.id');
                  ranMigrationsIds = _.keys(_this._ranMigrations);
                  differenceCount = _.difference(migrationFilesIds, ranMigrationsIds);
                  return done(null, differenceCount.length === 0);
                }
              });
            };
            return _this._dbReady.then(makeDiffWhenDbReady, done);
          }
        };
      })(this));
    };
